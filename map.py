...
[4주차 업데이트]
- Streamlit 웹 앱 기본 구조(탭 3개) 정식 구현함.
- 지도 탭 레이아웃을 width=1000 고정값에서 use_container_width=True로 변경하여
  화면 크기에 맞게 지도가 꽉 차도록 수정함.
- 주소 입력 → geopy(Nominatim)로 좌표 변환 후 사용자 위치를 지도 위에 별도 마커로
  표시하는 기능 추가함.
- 알고리즘 담당 함수 연동 전까지 사용할 목업 함수를
  route.py로 분리하여 구현함. 추후 실제 함수로 교체 예정임.
"""
