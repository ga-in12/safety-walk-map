...
# 이 파일 위치:
# safety-walk-map/route/shortest_safety_route.py
#
# parent       -> route
# parent.parent -> safety-walk-map
...
# 보행 네트워크 그래프 경로
graph_path = DATA_PROCESSED_DIR / "gwangjin.graphml"

# 최종 safety_score CSV 경로
grid_path = DATA_PROCESSED_DIR / "safety_score_result.csv"
...
MAP_OUTPUT_PATH = OUTPUT_DIR / "route_map.html"


# 지오코딩에 사용할 지역명 접미사
# 광진구 내 주소/장소명 정확도를 높이기 위함
...
def load_safety_grid():
    ...
    optional = [
        ...
        GRID_SIZE_COL,
        CCTV_COUNT_COL,
        STREETLIGHT_COUNT_COL,
        EMERGENCY_BELL_COUNT_COL,
        POLICE_STATION_COUNT_COL,
        POLICE_SUBSTATION_COUNT_COL,
        POLICE_BOX_COUNT_COL,
        GRADE_COL,
        GRADE_COLOR_COL,
    ]
...
# 주소 -> 좌표 변환(geocode_address)이나 지도 클릭 좌표 추출은
# UI/입력 계층에서만 처리
def find_route(
    G,
    orig_lat,
    ...
    fast = summarize_route(
        G,
        fast_route,
    )

    safe = summarize_route(
        G,
        safe_route,
    )
...
def visualize(
    ...
    folium.PolyLine(
        ...
        tooltip=(
            f"최단 경로: "
            f"{fast_route['distance_m']:.0f}m / "
            f"{fast_route['time_min']:.1f}분"
        ),
    ).add_to(m)
    ...
    folium.PolyLine(
        ...
        tooltip=(
            f"안전 경로: "
            f"{safe_route['distance_m']:.0f}m / "
            f"{safe_route['time_min']:.1f}분"
        ),
    ).add_to(m)
...
# 입력 계층:
# 주소/장소명(위경도값이 아닌 텍스트) -> 좌표 변환
# find_route()와는 분리
_geolocator = Nominatim(
    user_agent="gwangjin_safety_route"
)


def geocode_address(address: str):
    """
    주소 또는 장소명을 (위도, 경도)로 변환.
    실패 시 None 반환.
    """

    try:
        location = _geolocator.geocode(
            f"{address}, {GEOCODE_REGION_HINT}",
            timeout=5,
        )
...
def get_coords_from_user(label: str):
    ...
    Streamlit에서는 이 함수 대신 st_folium 클릭 좌표를
    그대로 find_route()에 넘기면 됨.
    """

    raw = input(
        f"{label} 입력 "
        f"(예: '37.5404,127.0700' "
        f"또는 '건대입구역'): "
    ).strip()
...
if __name__ == "__main__":
    ...
    orig_lat, orig_lon = get_coords_from_user(
        "출발지"
    )
    ...
    dest_lat, dest_lon = get_coords_from_user(
        "도착지"
    )
    ...
    print(
        f"총 거리 : "
        f"{result['fast']['distance_m']:.0f} m"
    )
    print(
        f"총 시간 : "
        f"{result['fast']['time_min']:.1f} 분"
    )
    ...
    print(
        f"총 거리 : "
        f"{result['safe']['distance_m']:.0f} m"
    )
    print(
        f"총 시간 : "
        f"{result['safe']['time_min']:.1f} 분"
    )

    print()
    print(
        f"시간 증가율 : "
        f"{result['time_increase_ratio'] * 100:.1f}%"
    )
    print(
        f"추천 경로 : "
